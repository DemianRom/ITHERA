...
// Sprint 8 — Integración financiera
import { registrarGastoDesdeProuesta } from '../budget/budget.service';

// -------------------------------------------------------
// Bloqueo de propuestas (Sprint 7)
// -------------------------------------------------------
...
export const updateProposal = async (proposalId: number, authUserId: string, payload: UpdateProposalPayload) => {
  ...
  // -------------------------------------------------------
  // Sprint 8 — Integración financiera
  // Si la propuesta acaba de ser aprobada, generar el gasto
  // automáticamente. El try/catch evita que un fallo en el
  // módulo financiero revierta la aprobación de la propuesta.
  // -------------------------------------------------------
  if (payload.estado === 'aprobada') {
    try {
      await registrarGastoDesdeProuesta(proposalId);
    } catch (err) {
      console.error('[Budget] Error al registrar gasto desde propuesta:', err);
    }
  }
